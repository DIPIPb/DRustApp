...
async function state(req, res) {
  ...
  if (serverInfo.world_size || serverInfo.level || serverInfo.hostname) {
    await sql`
      update servers set
        world_size = coalesce(${serverInfo.world_size || null}, world_size),
        seed = coalesce(${serverInfo.seed ?? null}, seed),
        level = coalesce(${serverInfo.level || null}, level),
        hostname = coalesce(${serverInfo.hostname || null}, hostname),
        version = coalesce(${serverInfo.version || null}, version),
        active_count = coalesce(${serverInfo.active_count ?? null}, active_count),
        joining_count = coalesce(${serverInfo.joining_count ?? null}, joining_count),
        queued_count = coalesce(${serverInfo.queued_count ?? null}, queued_count),
        reserved_count = coalesce(${serverInfo.reserved_count ?? null}, reserved_count),
        slots = coalesce(${serverInfo.slots ?? null}, slots),
        map_image_url = coalesce(${serverInfo.level_image_url || null}, map_image_url),
        last_seen_at = now()
      where id = ${server.id}
    `;

    // История для графика — сэмплируем раз в 5 минут, чтобы не раздувать
    // таблицу (плагин шлёт стейт каждые 5 секунд).
    if (serverInfo.active_count !== undefined) {
      const { rows: lastSnap } = await sql`
        select created_at from server_stats_history
        where server_id = ${server.id}
        order by created_at desc limit 1
      `;
      const shouldSample = !lastSnap[0] ||
        (Date.now() - new Date(lastSnap[0].created_at).getTime()) > 5 * 60 * 1000;

      if (shouldSample) {
        await sql`
          insert into server_stats_history (server_id, active_count, joining_count, queued_count, slots)
          values (${server.id}, ${serverInfo.active_count ?? 0}, ${serverInfo.joining_count ?? 0}, ${serverInfo.queued_count ?? 0}, ${serverInfo.slots ?? 0})
        `;
      }
    }
  } else {
    await sql`update servers set last_seen_at = now() where id = ${server.id}`;
  }

  // Плагин шлёт position в формате Unity Vector3.ToString(), например
  // "(123.4, 45.0, -67.8)" — вытаскиваем x/z для отрисовки на карте.
  function parsePosition(position) {
    if (!position) return { x: null, z: null };
    const match = /\(?\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)\s*,\s*(-?[\d.]+)\s*\)?/.exec(position);
    if (!match) return { x: null, z: null };
    return { x: parseFloat(match[1]), z: parseFloat(match[3]) };
  }

  for (const p of players) {
    const { x, z } = parsePosition(p.position);
    await sql`
      insert into players (server_id, steam_id, steam_name, ip, status, ping, is_alive, pos_x, pos_z, coords, can_build, is_raiding, meta, last_seen_at)
      values (${server.id}, ${p.steam_id}, ${p.steam_name || null}, ${p.ip || null}, ${p.status || null}, ${p.ping || null}, ${p.is_alive ?? null}, ${x}, ${z}, ${p.coords || null}, ${p.can_build ?? null}, ${p.is_raiding ?? null}, ${JSON.stringify(p.meta || {})}, now())
      on conflict (server_id, steam_id) do update set
        ...
        is_raiding = excluded.is_raiding,
        meta = excluded.meta,
        last_seen_at = now()
    `;
  }
